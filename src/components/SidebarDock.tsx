...
import { 
    ...
    Clapperboard, Mic2, Settings, ScrollText, Check, ChevronRight, LayoutTemplate,
    UserCircle, Shield, User, Zap
} from 'lucide-react';
import { NodeType, Workflow, Character } from '@/types';
...
const getNodeIcon = (t: string) => {
    switch(t) {
        ...
        case NodeType.STORYBOARD_GENERATOR: return LayoutTemplate;
...
export const SidebarDock: React.FC<SidebarDockProps> = ({
    onAddNode, onUndo, isChatOpen, onToggleChat, isMultiFrameOpen, onToggleMultiFrame, isSonicStudioOpen, onToggleSonicStudio, assetHistory, onHistoryItemClick, onDeleteAsset, workflows, selectedWorkflowId, onSelectWorkflow, onSaveWorkflow, onDeleteWorkflow, onRenameWorkflow, onImportWorkflow, characters, onAddCharacter, onDeleteCharacter, onApplyCharacter, onOpenSettings, onOpenAdmin, isReadOnly
}) => {
    const [activePanel, setActivePanel] = useState<'nodes' | 'workflow' | 'history' | 'characters' | null>(null);
    const [editingWorkflowId, setEditingWorkflowId] = useState<string | null>(null);
    const [tempTitle, setTempTitle] = useState('');
    const [isLoggedIn, setIsLoggedIn] = useState(false);

    const renderAuthMockup = () => (
        <div className="px-3 mb-6">
            {!isLoggedIn ? (
                <button 
                    onClick={() => setIsLoggedIn(true)}
                    className="w-full p-3 bg-gradient-to-br from-indigo-600 to-purple-700 rounded-2xl border border-white/10 shadow-lg hover:scale-[1.02] transition-all group overflow-hidden relative"
                >
                    ...
                    <div className="flex items-center gap-3 relative z-10">
                        ...
                        <div className="text-left">
                            <p className="text-[10px] font-black text-white uppercase tracking-widest">登录 / 注册</p>
                            <p className="text-[8px] text-white/60 font-medium">解锁云端同步与高级功能</p>
                        </div>
                    </div>
                </button>
            ) : (
                <div className="w-full p-3 bg-white/5 rounded-2xl border border-white/5 flex items-center justify-between group">
                    <div className="flex items-center gap-3">
                        <div className="relative">
                            <img src="https://api.dicebear.com/7.x/avataaars/svg?seed=Felix" className="w-8 h-8 rounded-full bg-indigo-500/20" />
                            <div className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 bg-emerald-500 border-2 border-[#0c0c0e] rounded-full" />
                        </div>
                        <div className="text-left">
                            <p className="text-[10px] font-black text-white uppercase tracking-widest">Creator #882</p>
                            <div className="flex items-center gap-1.5 mt-0.5">
                                <Zap size={8} className="text-amber-400" />
                                <span className="text-[8px] text-slate-500 font-bold uppercase tracking-widest">Pro Plan • 850 Credits</span>
                            </div>
                        </div>
                    </div>
                    <button onClick={() => setIsLoggedIn(false)} className="p-1.5 text-slate-600 hover:text-white opacity-0 group-hover:opacity-100 transition-all"><X size={14}/></button>
                </div>
            )}
        </div>
    );
    ...
    const renderCharactersPanel = () => (
        <div className="flex flex-col h-full bg-[#0c0c0e]/95 backdrop-blur-xl border-r border-white/5 w-72 animate-in slide-in-from-left duration-300">
            <div className="p-4 border-b border-white/5 flex items-center justify-between">
                <div className="flex items-center gap-2">
                    <UserCircle size={18} className="text-purple-400" />
                    <h3 className="text-xs font-black uppercase tracking-widest text-white">一致性角色库</h3>
                </div>
                <button onClick={() => setActivePanel(null)} className="p-1 hover:bg-white/5 rounded-lg text-slate-500"><X size={16}/></button>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
                {renderAuthMockup()}
                {characters.length === 0 ? (
                    <div className="text-center py-10">
                        <UserCircle size={32} className="mx-auto text-slate-700 mb-2 opacity-20" />
                        <p className="text-[10px] text-slate-500">尚无角色。从生成结果中保存角色以保持视觉一致性。</p>
                    </div>
                ) : (
                    characters.map(char => (
                        <div key={char.id} className="group relative bg-white/5 rounded-xl overflow-hidden border border-white/5 hover:border-purple-500/30 transition-all">
                            <img src={char.image} className="w-full h-32 object-cover opacity-80 group-hover:opacity-100 transition-opacity" />
                            <div className="p-3 bg-gradient-to-t from-black/80 to-transparent absolute bottom-0 left-0 right-0">
                                <h4 className="text-[11px] font-bold text-white truncate">{char.name}</h4>
                                <p className="text-[9px] text-slate-400 truncate mt-0.5">{char.description}</p>
                            </div>
                            <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button 
                                    onClick={() => onApplyCharacter(char)}
                                    className="p-1.5 bg-purple-500 text-white rounded-lg shadow-lg hover:scale-105 transition-all"
                                    title="应用到当前创作"
                                >
                                    <Check size={12} />
                                </button>
                                <button 
                                    ...
                                    className="p-1.5 bg-black/60 text-red-400 rounded-lg hover:bg-red-500/20 transition-all"
                                >
                                    <Trash2 size={12} />
                                </button>
                            </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-2 custom-scrollbar space-y-2">
                        ...
                        {isReadOnly ? (
                            <div className="text-center py-10 px-4">
                                <Box size={32} className="mx-auto text-slate-700 mb-2 opacity-20" />
                                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">预览模式下无法添加节点</p>
                            </div>
                        ) : (
                            [NodeType.PROMPT_INPUT, NodeType.SCRIPT_MASTER, NodeType.IMAGE_GENERATOR, NodeType.STORYBOARD_GENERATOR, NodeType.VIDEO_GENERATOR, NodeType.VIDEO_ANALYZER, NodeType.IMAGE_EDITOR, NodeType.AUDIO_GENERATOR].map(t => {
            ...
            return (
                <>
                    ...
                    <div className="p-3">
                        ...
                        <label className="w-full py-3 mt-2 bg-white/5 hover:bg-white/10 text-slate-300 rounded-xl font-bold text-[11px] uppercase tracking-wider flex items-center justify-center gap-2 transition-all cursor-pointer border border-white/5 hover:border-white/10">
                            <FolderPlus size={14} /> 导入工作流 (JSON)
                            <input 
                                type="file" 
                                accept=".json"
                                className="hidden"
                                onChange={(e) => {
                                    ...
                                    reader.onload = (ev) => {
                                        try {
                                            ...
                                            if (!wf.id || !wf.nodes || !wf.connections) {
                                                alert("无效的工作流文件");
                                                return;
                                            }
                                            // Ensure ID is unique to avoid collision with existing
                                            wf.id = `imported-${Date.now()}`;
                                            wf.title = `${wf.title} (Imported)`;
                                    ...
                                    <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button 
                                            onClick={(e) => { 
                                                ...
                                                a.href = url;
                                                a.download = `inkai-workflow-${wf.title.replace(/\s+/g, '_')}.json`;
                                            ...
                                            title="导出为 JSON"
                                        >
                                            <FolderPlus size={12} className="rotate-180" />
                                        </button>
    ...
    return (
        <>
            <div className="fixed left-6 top-1/2 -translate-y-1/2 flex flex-col items-center gap-3 p-2 bg-[#1c1c1e]/80 backdrop-blur-3xl border border-white/10 rounded-2xl shadow-2xl z-50 animate-in slide-in-from-left-10 duration-500 ease-[${SPRING}]">
                {[
                    { id: 'nodes', icon: Plus, action: () => setActivePanel(activePanel === 'nodes' ? null : 'nodes') },
                    ...
                    { id: 'characters', icon: UserCircle, action: () => setActivePanel(activePanel === 'characters' ? null : 'characters') },
                    ...
                    { id: 'admin', icon: Shield, action: onOpenAdmin, color: 'text-amber-500' },
