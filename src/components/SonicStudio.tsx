...
import { 
    ...
    FileText, MessageCircle, Activity, AudioLines, FolderPlus, Trash2
} from 'lucide-react';
...
export const SonicStudio: React.FC<SonicStudioProps> = ({ isOpen, onClose, history, onGenerate, onSaveToAssets }) => {
    ...
    // --- Visualizer Logic ---
    useEffect(() => {
        if (isPlaying && canvasRef.current && audioRef.current) {
            ...
            <div className="w-64 h-full border-r border-white/5 bg-[#121214] flex flex-col z-10">
                ...
                <div className="flex flex-col gap-1 p-4">
                    ...
                    <button 
                        ...
                        className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-bold transition-all ${isMixerOpen ? 'bg-cyan-500/20 text-cyan-400' : 'text-slate-400 hover:bg-white/5 hover:text-white'}`}
                    >
                        <Activity size={16} /> 多轨混音器 (Multi-track)
                    </button>
                </div>
            ...
            <div className="flex-1 flex flex-col relative z-0">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-8 pb-32">
                    <div className="max-w-5xl mx-auto flex flex-col gap-8">
                        ...
                        {/* --- MIXER OVERLAY --- */}
                        {isMixerOpen && (
                            <div className="bg-[#121214] border border-white/10 rounded-2xl p-6 shadow-2xl animate-in fade-in slide-in-from-right-10 duration-500">
                                <div className="flex items-center justify-between mb-6">
                                    <div className="flex items-center gap-2">
                                        <Activity size={18} className="text-cyan-400" />
                                        <h2 className="text-sm font-black uppercase tracking-widest text-white">多轨混音器 (Mixer)</h2>
                                    </div>
                                    <button onClick={() => setIsMixerOpen(false)} className="text-slate-500 hover:text-white"><X size={18}/></button>
                                </div>
                                <div className="space-y-4">
                                    {tracks.length === 0 ? (
                                        <div className="py-10 text-center border border-dashed border-white/5 rounded-xl">
                                            <p className="text-[10px] text-slate-600 uppercase font-bold tracking-widest">从历史记录中拖入音频以混音</p>
                                        </div>
                                    ) : (
                                        tracks.map((track, i) => (
                                            <div key={i} className="bg-white/5 p-4 rounded-xl flex items-center gap-4 border border-white/5">
                                                <div className={`w-10 h-10 rounded-lg bg-gradient-to-br ${getRandomCover(track.id)} flex items-center justify-center shrink-0`}>
                                                    <Mic2 size={16} className="text-white/70" />
                                                </div>
                                                <div className="flex-1 min-w-0">
                                                    <div className="text-[11px] font-bold text-white truncate">{track.title}</div>
                                                    <div className="flex items-center gap-3 mt-2">
                                                        <Volume2 size={12} className="text-slate-500" />
                                                        <input type="range" className="flex-1 h-1 bg-white/10 rounded-full appearance-none accent-cyan-500" />
                                                    </div>
                                                </div>
                                                <button onClick={() => setTracks(prev => prev.filter((_, idx) => idx !== i))} className="p-2 text-slate-600 hover:text-red-400"><Trash2 size={16}/></button>
                                            </div>
                                        ))
                                    )}
                                </div>
                ...
                {(activeTab === 'factory' || (currentTrack && activeTab !== 'live')) && (
                    <div className="h-24 bg-[#121214]/90 backdrop-blur-xl border-t border-white/10 flex items-center px-8 gap-8 relative z-20">
                        ...
                        <div className="w-64 flex items-center gap-4">
                            <div className={`w-14 h-14 rounded-lg bg-gradient-to-br ${currentTrack ? getRandomCover(currentTrack.id) : 'from-slate-700 to-slate-800'} shadow-lg flex items-center justify-center relative overflow-hidden group/cover`}>
                                {currentTrack ? <Mic2 size={24} className="text-white/80" /> : <Disc size={24} className="text-slate-500" />}
                                <canvas ref={canvasRef} className="absolute inset-0 w-full h-full opacity-40" width={56} height={56} />
                            </div>
                        ...
                        <div className="w-64 flex items-center justify-end gap-3">
                            <div className="flex items-center gap-2 group/vol">
                                ...
                                <div className="w-16 h-1 bg-white/10 rounded-full relative cursor-pointer">
                                    <div className="absolute top-0 left-0 h-full bg-slate-400 group-hover/vol:bg-white rounded-full" style={{ width: `${volume * 100}%` }} />
                                </div>
                            </div>
                            <button 
                                onClick={() => currentTrack && onSaveToAssets?.({ ...currentTrack, type: 'audio' })}
                                className="p-2 rounded-lg bg-white/5 hover:bg-purple-500/20 text-slate-400 hover:text-purple-400 transition-all"
                                title="保存到素材库"
                            >
                                <FolderPlus size={18} />
                            </button>
