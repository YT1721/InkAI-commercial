...
import { 
    X, Brush, Eraser, Palette, Undo, Redo, Trash2, 
...
export const SketchEditor: React.FC<SketchEditorProps> = ({ onClose, onGenerate }) => {
    ...
    // Redo Logic
    const [canvasRedoHistory, setCanvasRedoHistory] = useState<ImageData[]>([]);
    ...
    const handleUndo = () => {
        ...
        if (canvas && ctx) {
             ...
             setCanvasRedoHistory(prev => [...prev, currentData]);
        }
    ...
    const handleRedo = () => {
        if (canvasRedoHistory.length === 0) return;
        
        const newRedoHistory = [...canvasRedoHistory];
        const nextState = newRedoHistory.pop();
        setCanvasRedoHistory(newRedoHistory);

        const canvas = canvasRef.current;
        const ctx = canvas?.getContext('2d');
        if (canvas && ctx && nextState) {
            ctx.putImageData(nextState, 0, 0);
            // Add back to history so we can undo again
    ...
    }, [canvasHistory, canvasRedoHistory]); // Re-bind when history changes to get latest handlers
    ...
    const getCompositeDataURL = (): string => {
        ...
        if (backgroundImage) {
            ...
            // UI uses object-contain. So we need to calculate dimensions.
                ...
                <div className="absolute top-12 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2 p-1.5 bg-[#2c2c2e]/90 backdrop-blur-xl border border-white/10 rounded-full shadow-2xl">
                    ...
                    <div className="flex items-center gap-2 px-3 py-1.5 bg-black/20 rounded-full border border-white/5">
                        ...
                        <input 
                            ...
                            className="w-20 h-1 bg-white/10 rounded-full appearance-none [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-cyan-500 cursor-pointer"
                        />
                        <span className="text-[10px] font-mono text-slate-400 min-w-[20px]">{tool === 'eraser' ? eraserSize : brushSize}</span>
                    </div>
                    ...
                    <button 
                        ...
                        className={`p-2.5 rounded-full transition-colors ${canvasHistory.length <= 1 ? 'opacity-20 cursor-not-allowed' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
                        title="撤销 (Ctrl+Z)"
                    >
                        <Undo size={16} />
                    </button>
                    ...
                    <button 
                        ...
                        className={`p-2.5 rounded-full transition-colors ${canvasRedoHistory.length === 0 ? 'opacity-20 cursor-not-allowed' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
                        title="重做 (Ctrl+Y)"
                    >
                        <Redo size={16} />
                    </button>
                    
                    <button onClick={handleClear} className="p-2.5 rounded-full text-red-400 hover:bg-red-500/10" title="清空画布">
                        <Trash2 size={16} />
                    </button>
                </div>
